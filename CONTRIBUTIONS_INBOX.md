...
## Entries
### 2026-09-21 — Orchestration doctrine can silently outlive the harness limitation it was written for (and a plausible mechanism is not a verified one)

- **Trigger:** a cross-project orchestration skill carried a rule mandating a specific workaround — spawn every sub-agent in the foreground, never backgrounded — justified by a dated finding that the harness had no way to message a spawned agent once it was running. Ten days later, on the same class of session, that capability was re-tested and found working, but the workaround had kept being followed as current doctrine in the interval, not because anyone re-confirmed it still applied but because nothing forced a re-check. A second, layered mistake happened live during the correction itself: the FIX that replaced the stale rule was first drafted around a plausible-sounding mechanism ("a background child dies the instant its bounded spawner's task ends") that had never actually been checked against the vendor's documentation. Fetching the live docs found a different and more nuanced reality — one child type is hard-blocked from backgrounding by the platform itself (with a stated reason), a second type is allowed to background but only ends up blocked-on or mis-routed rather than lost, and a third, more severe case (the top-level session's own host process exiting) is genuinely undocumented and rests only on an operator-witnessed incident. The practical advice ("default nested spawns to foreground") happened to survive the correction, but the REASON attached to it did not, and would have shipped wrong if nobody had checked.
- **Is it generic?** Yes, on two independent axes. First, **the tell that doctrine is stale is structural, not textual**: a rule that mandates a workaround and cites a DATED capability finding as its justification is, by construction, evidence that a capability was believed absent as of that date — and a belief about capability needs a cheap re-verification step attached to it, not a permanent rule copied forward from memory. Second, **a corrected rule needs the same verification discipline as the rule it replaces**: "that sounds right" and "that matches the one incident I remember" are not the same as checking the current, authoritative source, and a plausible mechanism dressed up with a specific-sounding detail ("dies the instant," "orphaned") reads as more verified than it is. Both kernels strip cleanly: the specific harness, tool names, dates, and project are incidental: any system with (a) capability-gated doctrine and (b) a documentation source of truth has both failure modes available to it.
- **Target:** a new tagged file under `lessons/`, e.g. `lessons/verify-dont-remember-harness-capability.md` (tags: orchestration, capability-drift, agent-harness, stale-doctrine, verification).
- **Proposed change:**

  > **Verify a harness capability every session; never carry forward a remembered answer, in either direction — and verify a CORRECTION the same way you'd verify the rule it replaces.**
  >
  > When orchestration doctrine encodes "do X because capability Y is unavailable," treat that as a claim with a shelf life, not a fact. The tell that a rule has gone stale is structural: it mandates a workaround AND cites a dated finding as its justification. Attach a cheap, repeatable probe to any such rule instead of a permanent instruction:
  >
  > 1. Spawn one throwaway worker in the mode you'd use if the capability existed (backgrounded/named/addressable).
  > 2. Send it a trivial round-trip (e.g. "reply with nonce {{RANDOM}}").
  > 3. If it replies, the capability is live — proceed under the normal-case doctrine. If not, fall back to the workaround for this session only.
  >
  > One probe, done once at session start, replaces a permanently-mandated workaround and a permanently-assumed capability alike — neither "it's broken" nor "it's fixed" should survive from a prior session's notes without being re-checked.
  >
  > **The second kernel, easy to miss because it hides inside the act of fixing the first problem:** when you replace stale doctrine, the new doctrine's stated MECHANISM is itself a claim that needs a source, not just the new PRACTICAL ADVICE. A plausible-sounding "why" — generalized from one real incident, or from how a similar system behaves elsewhere — is not the same as the vendor's or codebase's actual documented behavior, even when it produces the same recommendation. Concretely: "background work dies when its spawner's bounded task ends" turned out to conflate three genuinely different cases (a hard platform restriction with a stated reason; an allowed-but-pointless case that blocks or mis-routes rather than loses the work; and a real but explicitly UNDOCUMENTED case resting on one incident). The advice ("default to foreground here") survived; the reason did not, and shipping the wrong reason teaches the next reader to misapply the rule the moment the surface details differ from the one incident it was drawn from.
  >
  > **The depth-dependent shape itself, stated without over-claiming the mechanism:** when doctrine says "background a spawned worker by default," the safe default is not uniform across the whole system — it depends on whether the SPAWNER outlives the unit of work. A spawner that persists across turns/requests (a long-lived orchestrator) can reasonably background its children, since it is still around later to collect the result. A spawner that is itself bounded by a single task is a different case whose actual behavior (blocked, refused, mis-routed, or genuinely lost) is platform-specific — look it up for your framework rather than assuming any one of those outcomes, and default such a spawner's own spawns to the foreground regardless, since none of the documented outcomes give it a parallelism benefit worth the doctrinal risk. State that default explicitly in any brief that hands a worker the ability to spawn further agents — a worker inherits no orchestration doctrine it wasn't told.
