...
- Related: [[guard-hooks-deny-teach-ack]] (how to build the guard), [[migrated-config-carries-source-host-env]] (why the config still describes the old machine), and [[a-gate-that-exists-vs-a-gate-that-covers]] (the same coverage question for a build-time guard, where the uncovered surface is a deploy script rather than a config file).
