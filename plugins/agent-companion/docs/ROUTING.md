...
| `integration` | 4 | `bounded` | `elevated` | `opus/medium` _(trial override)_ | multi-file, cross-referencing, touches shared config or things other agents depend on |
...
- **`integration`** — Our weight scale. Consequence elevated because shared surfaces are where a mistake costs other people time. UNBENCHMARKED by the 2026-09-23 effort-grid trial itself, but see operatorNote and the ROUTING TRIAL v3 override below.
...
| `integration` | `opus/medium` | `sonnet/high` | 2026-09-24 | 2026-09-30 | operator architecture-effort decision, DECISIONS.md (kept outside this repo) (2026-09-24) |
...
- **`integration`** — ROUTING TRIAL v3 (architecture-effort operator decision, DECISIONS.md; supersedes v2's opus/high): still unmeasured by benchmark, but the operator judged v2's opus/high heavier than integration work needs day to day, and moved it down one rung to opus/medium. Model stays opus: operator first-hand evidence says Sonnet struggles on some of the operator's technical, multi-file work (operator-observed 2026-09-23) -- do not route this off Opus on benchmark evidence alone. opus/medium is BELOW the elevated-consequence effort floor (F5, high) -- rather than lowering that floor for every elevated route, this row carries its own explicit F5 waiver (waivesFloor/source, the same shape a per-user routing-profile row uses to waive F5), honoured because the evidence behind it is the operator's own first-hand observation, not a benchmark. Only this row is waived: the elevated floor otherwise stays at high for every grid-path elevated route, every other declared CONSEQUENCE: elevated, and any future trial row that does not carry its own honoured waiver. large-refactor and novel-design stay at opus/high, already at the floor. critical-change is unaffected (F1/xhigh, not F5). A separate, non-waivable rule (F6) still refuses opus/low for any architecture-class type regardless of this waiver.
