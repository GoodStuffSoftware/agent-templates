...
|---|---|---|---|
| `routine` | — | — | — |
| `elevated` | `medium` | — | shared config, build or release pipeline, anything other agents depend on, public API shape |
...
- **`integration`** — ROUTING TRIAL v3 (architecture-effort operator decision, DECISIONS.md; supersedes v2's opus/high): still unmeasured by benchmark, but the operator judged v2's opus/high heavier than integration work needs day to day, and moved it down one rung to opus/medium. Model stays opus: operator first-hand evidence says Sonnet struggles on some of the operator's technical, multi-file work (operator-observed 2026-09-23) -- do not route this off Opus on benchmark evidence alone. Landing on medium (not v2's high) required lowering the elevated-consequence effort floor from high to medium (config/model-tiers.json consequence.elevated.effortFloor) -- at the old high floor, F5 would have raised any elevated-consequence trial straight back to high regardless of what this override named, which is exactly why v2 could only change the model, not the effort. large-refactor and novel-design deliberately stay at opus/high (unaffected by the lower floor, since high already sits above medium); critical-change is unaffected (F1/xhigh, not F5). A separate, non-waivable rule (F6) still refuses opus/low for any architecture-class type -- this trial does not touch that floor.
