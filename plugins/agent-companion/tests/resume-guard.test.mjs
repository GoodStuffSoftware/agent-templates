...
// Ladder track "rungttl" (2026-09-26 DECISIONS.md): ac-opus-medium/high/
// xhigh/max now carry experimental.cacheTtl: "1h" in their shipped
// agents/ac-*.md frontmatter (config/model-tiers.json's `ladder[].cacheTtl`,
// generated by scripts/routing-table.mjs); ac-opus-low and every other rung
// do not. No fixture needed: both the bare and the `agent-companion:`-
// namespaced form resolve through agentDefinition()'s own ladder/self-plugin
// shortcuts straight to the real, committed files, so this reads exactly
// what a live resume-guard hook would resolve.
test('lib: cacheTtlFromDefinition resolves agent-companion:ac-opus-high to "1h" and ac-opus-low to null (falls back to the 5m default)', () => {
  assert.equal(cacheTtlFromDefinition('agent-companion:ac-opus-high'), '1h');
  assert.equal(cacheTtlFromDefinition('ac-opus-high'), '1h', 'bare form resolves the same way');
  assert.equal(cacheTtlFromDefinition('agent-companion:ac-opus-medium'), '1h');
  assert.equal(cacheTtlFromDefinition('agent-companion:ac-opus-xhigh'), '1h');
  assert.equal(cacheTtlFromDefinition('agent-companion:ac-opus-max'), '1h');
  ...
  assert.equal(cacheTtlFromDefinition('agent-companion:ac-opus-low'), null);
  assert.equal(cacheTtlFromDefinition('ac-opus-low'), null, 'bare form resolves the same way');
});
