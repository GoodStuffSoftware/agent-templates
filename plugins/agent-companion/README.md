...
| `capacity_probe` | At session start, a one-line estimate of how many concurrent subagents this machine can carry right now (free memory / cpu count -> a concurrency budget and an idle-teammates-ok vs stop-between-rounds policy). Read-only, cheap, wrapped so a slow or failing probe never blocks the hook. `scripts/capacity.mjs` also runs standalone (`--text`, `--per-agent-mb`, `--headroom-gb`, `--threshold`). | no |
