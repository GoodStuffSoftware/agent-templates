...
| `state/premium-window.json` | state root | rolling window used to approximate premium concurrency; read-modify-written only under `premium-window.json.lock` (a transient O_EXCL lock both hooks share; a stale one is broken after 5 s) |
