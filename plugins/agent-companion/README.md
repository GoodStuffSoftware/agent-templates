...
| `state/baseline.json` | state root | previous harness version + counters, for daily drift detection; also the publication-leak sweep's per-repo hit fingerprints, so an accepted finding doesn't re-fire daily |
