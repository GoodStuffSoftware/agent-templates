...
//   node transcript-report.mjs --max-ms 20000     # stop reading after a time budget
//
...
console.log(`files: ${s.filesRead} read of ${s.filesFound} found (${s.mainFiles} main, ${s.subagentFiles} subagent), `
  + `${s.filesWithRequestsInWindow} with requests in window, ${(s.wallMs / 1000).toFixed(1)}s${s.truncated ? ' — TRUNCATED' : ''}`);
...
for (const b of report.resumeGaps) {
  if (!b.count) continue;
  console.log(`  ${b.label.padEnd(7)} ${String(b.count).padEnd(8)} hit ${String(b.hits).padEnd(8)} rewrite ${String(b.rewrites).padEnd(6)} `
    + `rewritten ${m(b.rewriteTokens).padEnd(8)} after-compaction ${b.afterCompaction}`);
}
console.log('');
