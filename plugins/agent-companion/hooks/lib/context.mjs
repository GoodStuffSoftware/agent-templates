...
// --- installed_plugins.json readers ----------------------------------------
// Shared by the spawn guard (which stamps its own install scope into every
// spawns.jsonl row), agentDefinition() (another plugin's agents/ folder), the
// SessionStart ladder check and the daily scout (which compares the guard
// version a spawn ran under against what is installed for that same scope).
// Kept in this file rather than a module of their own because every hook
// loads context.mjs, and each extra module file costs every hook load
// (tests/routing-profile-timing.test.mjs budgets that path). Every function
// takes the paths it needs as arguments; nothing here throws, and an
// unreadable or malformed file reads as "no installs known", which every
