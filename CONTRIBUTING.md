...
**Release gate:** a release isn't done when the release branch's own CI is green — it's done when GitHub CI is green on the release branch **and** on `main` after the push. (If this repo grows a dedicated release doc or skill, this gate belongs there too, cross-referenced from here rather than restated.)
