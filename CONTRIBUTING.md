...
- **scans every commit being pushed, on every ref — `wip/**` and `backup/**` included** (`scripts/push-scan.mjs`). Each commit's added lines, message and touched paths go through leak-check's own checks and through your local private-names denylist, `~/.claude/agent-companion/config/private-names.txt` (or `$AGENT_COMPANION_STATE_DIR/config/private-names.txt`). A hit blocks the push and prints only the commit SHA, the file and line (or "commit message" / "touched path #N"), and which check fired. It never prints the matched text. Denylist format: one entry per line, `#` comments allowed. A plain entry matches case-insensitively as a whole word, so `ann` does not match `annotation`. An entry starting `re:` is a case-insensitive regex. If the file is missing, the hook prints a one-line warning and carries on. An invalid `re:` entry blocks the push and names the line to fix. Author and committer names and emails are not scanned. A SHA in a commit message is allowed when it names a commit in this repository, as `git revert` messages do;
